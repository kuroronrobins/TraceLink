 ...
 * 実装側では reader、session、結果登録、pending write、log を組み合わせるが、
 * UI からはその詳細を見せない。
 */
interface InventoryRepository : AutoCloseable {
    ...
    suspend fun registerCurrentSessionResults()
    suspend fun retryPendingWrites()
